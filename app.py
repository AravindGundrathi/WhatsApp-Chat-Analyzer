...
if uploaded_file is not None:
    ...
    if st.sidebar.button('Summary'):
        ...
        st.markdown(
            f"""
            <div style="background-color:#f3ece9; padding:20px; border-radius:5px; font-size:17px; margin-bottom:60vh">
                {summarized_text}
            </div>
            """,
            unsafe_allow_html=True
...
# The rest of your code continues here...
    ...
    # Perform sentiment analysis
            output, pos_count, neg_count = sentimentAnalysis.perform_sentiment_analysis(chat_content)
            st.markdown("""
                <style>
                    .title {
                        font-size: 3em;
            ...
            if output:
                ...
                st.markdown(f"<div style='margin-bottom:40vh' class='box'><span class='normalText'>Total Positives : <span class='positive'>{pos_count}</span> </span> <span class='normalText' style='margin-left:330px;'>Total Negatives : <span class='negative'>{neg_count}</span> </span> </div>", unsafe_allow_html=True)
                pdf_content = [
    ...
    if st.sidebar.button("Show Analysis"):                   # button
